...
const AppState = {
    cart: [],
    // cart persisted in localStorage under 'furnmart_cart'
...
function loadCartFromStorage() {
    try {
        const raw = localStorage.getItem('furnmart_cart');
        if (raw) {
            const parsed = JSON.parse(raw);
            if (Array.isArray(parsed)) {
                AppState.cart = parsed.map(item => ({
                    name: item.name,
                    price: item.price,
...
function dispatchCartUpdated() {
    document.dispatchEvent(new CustomEvent('cart:updated', { detail: { cart: AppState.cart } }));
}
// ============ DOM Elements ============
const elements = {
...
function initCart() {
    ...
    elements.addToCartBtns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            const productCard = btnEl.closest('.product-card');
            ...
            const productPrice = priceEl ? priceEl.textContent.trim() : 'P 0';
...
function addToCart(product) {
  ...
  // Normalize input
  const name = product && product.name ? String(product.name).trim() : "Product";
  const price = product && product.price ? String(product.price).trim() : "P 0";
  ...
  if (existing) {
    ...
    AppState.cart.push({ name, price, quantity: qty });
  }

  // UI + persistence
  updateCartCount();              // updates badge
  saveCartToStorage();            // localStorage: 'furnmart_cart'
  showNotification(`${name} added to cart!`, 'success');

  // Announce total quantity, not distinct items
  const totalQty = AppState.cart.reduce((s, i) => s + (parseInt(i.quantity, 10) || 0), 0);
  announceToScreenReader(`${name} added to cart. Cart now has ${totalQty} items.`);

  // Micro animation on the cart button (if present)
  if (elements && elements.cartBtn) {
    elements.cartBtn.classList.add('bounce');
    setTimeout(() => elements.cartBtn.classList.remove('bounce'), 500);
  }

  // Notify any listeners (drawer re-render, etc.)
  dispatchCartUpdated();
}
...
function init() {
    ...
    window.AppState = AppState; // expose for other scripts
    window.addToCart = addToCart;
    window.removeFromCart = removeFromCart;
    window.setCartQuantity = setCartQuantity;

    // Load cart from storage before wiring UI
    loadCartFromStorage();
    updateCartCount();
...
// ============ Global delegated Add-to-Cart (failsafe) ============
function cartDelegatedHandler(e){
    var target = e.target && (e.target.closest ? e.target.closest('.btn-add-cart') : null);
    if (!target) return;
    try {
